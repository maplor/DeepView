...
from mad_gui_main.mad_gui.config import BaseSettings, BaseTheme
from mad_gui_main.mad_gui.plot_tools.labels import BaseEventLabel, BaseRegionLabel
from mad_gui_main.mad_gui.plugins.base import BasePlugin
from mad_gui_main.mad_gui.plugins.example import (
    EnergyCalculator,
    ExampleExporter,
    ExampleImporter,
...
from mad_gui_main.mad_gui.windows import MainWindow
...
def start_gui(
    ...
    # )
    form = MainWindow(
        data_dir=data_dir, settings=settings, theme=theme, plugins=plugins, labels=labels, events=events
    )
    form.show()
