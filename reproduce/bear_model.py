...
Bear model module
==================
Bear counterpart of `turtle_model.py`.  The bear dataset has a single
...
50, so:
  ...
  * classifier -> 10 bear behaviour classes (raw logits).

`forward(data, if_contrast=True)` returns (head_output, shared_32d_feature),
matching the umineko `SimpleNN` / turtle `TurtleNN` interface so the generic
...
# Pass input_dim explicitly to BearNN when using a non-50 window.
FEATURE_DIM = 128 * 6


class BearNN(nn.Module):
    def __init__(self, input_dim=FEATURE_DIM, number_classes=10):
        super(BearNN, self).__init__()
...
def freeze_encoders(model):
    """Bear freeze: train classifier only (single-encoder variant of the
