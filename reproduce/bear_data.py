"""
Bear data preprocessing module
===============================
Bear counterpart of `turtle_data.py` / `data_preprocessing.py`.  The bear
dataset is structured like turtle (raw npy arrays), but with different shapes
and class set:

  * data : (N, 3, 50)  -> 3 accelerometer channels (no pressure channel),
                          window length 50.
  * label: (N, 50)     -> per-timestep activity label inside each window.
  * 10 behaviour classes (0..9); the dataset is fully labelled (no -1 unknown),
    so the unknown-drop in `build_splits` is a no-op here.

Pipeline (mirrors the turtle reproduce):
1. Load `data/bear/data.npy` + `data/bear/label.npy`.
2. Majority-vote each window's per-timestep labels into a single label.
3. Drop unknown (-1) windows for the supervised split (bear has none).
...
# majority_value is identical to the umineko/turtle version; reuse it so
# behaviour matches exactly.
from data_preprocessing import majority_value


class data_loader_bear(Dataset):
    """Minimal 2-argument Dataset matching the notebook's usage:
    ``data_loader_bear(samples, labels)`` yielding ``(sample, label)``."""
...
# --------------------------------------------------------------------------- #
# label dictionaries (bear, from
# deepview/calculate_results/models/utils.py: labeldict_findstr_bear)
# --------------------------------------------------------------------------- #
labeldict_findstr = {
    -1: "unknown",
    0: "dig",
...
def load_data(data_path, label_path):
    """Return (data_b, label_b, vote_label) from the bear npy files.

    vote_label is the majority-voted per-window label.  Like turtle there is no
    foraging remap; the bear dataset is fully labelled so no -1 appears."""
...
def build_splits(data_b, vote_label, test_size=0.2, labeled_frac=0.01,
                 random_state=42):
    """Reproduce the turtle/umineko notebook's split logic for the bear data.

    Returns a dict with X_train_full, X_test, y_*, X_labeled, X_unlabeled, ...
    """
    # drop unknown (-1) windows (no-op for bear, kept for parity)
...
if __name__ == "__main__":
    ...
    dp = os.path.normpath(os.path.join(here, "..", "data", "bear", "data.npy"))
    lp = os.path.normpath(os.path.join(here, "..", "data", "bear", "label.npy"))
