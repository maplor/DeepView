"""Generate `reproduce_walkthrough_turtle.ipynb` — turtle-dataset counterpart
of `reproduce_walkthrough.ipynb`.  Same step-by-step structure, keeping every
intermediate result inline, but targeting the turtle dataset
(`data/turtle/data.npy` + `label.npy`): 3 accel channels, window length 200,
7 behaviour classes."""
import os
...
code("""DATA = os.path.normpath(os.path.join(HERE, '..', 'data', 'turtle', 'data.npy'))
...
与 classifier(7 类)。""")
code("""model = TurtleNN(number_classes=NUM_CLASSES).to(device)
...
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'reproduce_walkthrough_turtle.ipynb')
